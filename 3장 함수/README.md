...
- [3장 함수 - `함수 잘 만드는 법`](#3장-함수---함수-잘-만드는-법)
	...
	- [함수 잘 만드는 법 - 10. 반복하지 마라 (DRY 원칙)](#함수-잘-만드는-법---10-반복하지-마라-dry-원칙)
	- [함수 잘 만드는 법 - 11. 구조적 프로그래밍](#함수-잘-만드는-법---11-구조적-프로그래밍)
	- [함수 잘 만드는 법 - 12. 함수를 어떻게 짜죠?](#함수-잘-만드는-법---12-함수를-어떻게-짜죠)

## 함수 잘 만드는 법 - 1. 작게 만들어라!

함수를 만들 때 최대한으로 **`작게`** 만들자!

* 물리적인 행 수로 크기를 측정한다. - 20줄 이내 (4 ~ 5 줄이 최고)
...
2. 의존성 자석(dependency magnet) : 오류 코드의 등록/ 수정/ 삭제 가 일어날 경우 해당 오류 코드의 처리 코드도 처리해야 한다.
...
## 함수 잘 만드는 법 - 10. 반복하지 마라 (DRY 원칙)

모든 소프트웨어에서 **중복**은 모든 악의 근원이다. ***소스 코드에서 지속적으로 중복을 제거하려고 하자!!***

## 함수 잘 만드는 법 - 11. 구조적 프로그래밍
...
에츠허르 다익스트라(Edsger Dijkstra)의 구조적 프로그래밍의 원칙은 **모든 함수와 함수 내 모든 블록에 입구(Entry)와 출구(Exit)가 하나만 존재해야 한다**고 말했다.
* 함수는 `return 문`이 하나여야 한다.
* 루프(loop) 안에서 `break`, `continue`를 사용하면 안된다. 특히 `goto 문`은 **절대로 사용해서는 안된다.**

하지만, 위의 규칙은 함수가 아주 클 때만 상당한 이익을 제공한다.

함수를 작게 만들었다면 `return, break, continue`를 **여러 차례 사용해도 괜찮다.**

## 함수 잘 만드는 법 - 12. 함수를 어떻게 짜죠?

***끊임없이 다듬자.***

1. 처음에는 들여쓰기 단계 多, 중복된 루프, 긴 인수목록, 이름은 즉흥적, 코드는 중복

2. But 단위 테스트 코드는 필수적으로 만들고 통과!

3. 리팩토링(Refactoring) : 코드를 다듬고, 함수를 만들고, 이름을 바꾸고, 중복을 제거, 메서드를 줄이고, 순서를 바꾼다. 전체 클래스를 쪼개기도 한다.
