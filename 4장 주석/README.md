...
특별한 이유 없이 의무감으로 혹은 프로세스에서 하ㅣ라고 하니까 마지못해 주석을 단다면 전적으로 시간낭비다.
...
너무 당연한 사실을 언급하며 새로운 사실을 제공하지 못하는 주석 -> 같은 이야기를 중복하는 주석과 똑같다.
...
소스 코드 관리 시스템이 코드를 누가 작성했는지 다 기억하고 있으니까 저자 이름으로 코드를 오염시킬 필요가 없다.
...
**과감히 지우자!** 소스 코드 관리 시스템이 코드를 기억해줄 것이기 때문이다.
